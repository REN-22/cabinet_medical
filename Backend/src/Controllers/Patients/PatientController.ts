...
export async function CreateUsager(req: any, res: any): Promise<void> {
  try {
    ...
    const idUsager = await createUsager(
      num_secu,
      civilite,
      nom,
      prenom,
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      return res.status(409).json({
        message:
          'Un usager avec le même numéro de sécurité sociale existe déjà.',
      })
    }
...
export async function UpdateUsager(req: any, res: any): Promise<void> {
  try {
    ...
    const isUpdated = await updateUsager(
      id,
      numSecu,
      civilite,
      nom,
...
export async function GetUsagerById(req: any, res: any): Promise<void> {
  try {
    ...
    res.status(500).json({
      message:
        "Une erreur s'est produite lors de la récupération de l'usager par ID.",
    })
  }
}
...
export async function DeleteUsagerById(req: any, res: any): Promise<void> {
  try {
    ...
    res.status(500).json({
      message:
        "Une erreur s'est produite lors de la suppression de l'usager par ID.",
    })
  }
}
