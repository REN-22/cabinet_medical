...
try {
    const { id_usager, id_medecin, date_consult, heure_consult, duree_consult } = req.body

    if (!id_usager || !id_medecin || !date_consult || !heure_consult || !duree_consult) {
...
try {
    ...
    const isUpdated = await updateRendezVous(id, id_usager, id_medecin, date_consult, heure_consult, duree_consult)
    ...
    res.status(500).json({
        message: "Une erreur s'est produite lors de la mise à jour du rendez-vous.",
    })
}
}
...
try {
    ...
    res.status(500).json({
        message: "Une erreur s'est produite lors de la récupération du rendez-vous par ID.",
    })
}
}
...
try {
    ...
    console.error('Erreur lors de la suppression du rendez-vous par ID :', error)
    res.status(500).json({
        message: "Une erreur s'est produite lors de la suppression du rendez-vous par ID.",
    })
}
}
