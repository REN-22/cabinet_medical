...
export async function getHoursWorkedPerMedecin(req: Request, res: Response) {
  try {
    ...
    medecins.forEach((medecin: any) => {
      hoursWorkedPerMedecin[medecin.id_medecin] = `${medecin.nom} ${medecin.prenom} ${calculateHoursForMedecin(rendezVous, medecin.id_medecin)}`;
    });

    res.status(200).json(hoursWorkedPerMedecin);
  } catch (error) {
    res.status(500).json({ error: 'Error occurred while calculating hours worked per medecin' });
  }
}

function calculateHoursForMedecin(rendezVous: any[], medecinId: number): string {
  let totalHours = 0;

  const rendezVousMedecin = rendezVous.filter((rdv: any) => rdv.id_medecin === medecinId);
...
export async function getStatistiquesPatients(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ error: 'Une erreur s\'est produite lors du calcul des statistiques des patients' });
  }
}
