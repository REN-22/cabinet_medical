  ...
  try {
    ...
    if (!civilite || !nom || !prenom) {
      return res.status(400).json({
        message:
          'Requête invalide. Toutes les informations requises ne sont pas fournies.',
      })
    }
    ...
    console.error("Erreur lors de la creation d'un merecin : ", error)
    res.status(500).json({
      message:
        "Une erreur interne s'est produite lors de la création du médecin.",
    })
  }
}
...
export async function GetAllMedecins(req: any, res: any): Promise<void> {
  try {
    ...
    res
      .status(200)
      .json({
        message: 'La liste des médecins a été récupérée.',
        data: medecins,
      })
  } catch (error) {
    ...
    res.status(500).json({
      message:
        "Une erreur interne s'est produite lors de la récupération des médecins.",
    })
  }
}
